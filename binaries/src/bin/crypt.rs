...
fn encrypt(
    mut client: Client<Prio2Field>,
    input: Input,
    server_1_encrypted_share: Output,
    server_2_encrypted_share: Output,
) -> Result<()> {
    let input_fields = Prio2Field::byte_slice_into_vec(&input.contents()?)
...
fn decrypt(
    mut server_1: Server<Prio2Field>,
    mut server_2: Server<Prio2Field>,
    server_1_encrypted_share: Input,
    server_2_encrypted_share: Input,
    ...
    if pretty_print {
        ...
        decrypted_input
            .into_writer()?
            .write_all(&Prio2Field::slice_into_byte_vec(&reconstructed))
...
fn main() -> Result<()> {
    ...
    match options.command {
        Subcommand::Encrypt {
            ...
            let client: Client<Prio2Field> = Client::new(
                options.dimension,
