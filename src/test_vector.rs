...
/// along with a reference sum. The field is always [`Prio2Field`].
...
pub struct Priov2TestVector {
    ...
    pub reference_sum: Vec<Prio2Field>,
...
impl Priov2TestVector {
    ...
    pub fn new(dimension: usize, number_of_clients: usize) -> Result<Self, TestVectorError> {
        let mut client: Client<Prio2Field> =
            Client::new(dimension, server_1_public_key(), server_2_public_key())?;

        let mut reference_sum = vec![Prio2Field::zero(); dimension];
        ...
        for _ in 0..number_of_clients {
            ...
            let data: Vec<Prio2Field> = (0..dimension)
                .map(|_| Prio2Field::from(rng.gen_range(0..2)))
                .collect();
    ...
    /// vector's servers.
    pub fn client(&self) -> Result<Client<Prio2Field>, TestVectorError> {
        Ok(Client::new(
            self.dimension,
    ...
    pub fn server_1(&self) -> Result<Server<Prio2Field>, TestVectorError> {
        Ok(Server::new(
            self.dimension,
            true,
    ...
    pub fn server_2(&self) -> Result<Server<Prio2Field>, TestVectorError> {
        Ok(Server::new(
            self.dimension,
            false,
...
mod base64 {
    ...
    use crate::field::{FieldElement, Prio2Field};
    ...
    pub fn serialize_field<S: Serializer>(v: &[Prio2Field], s: S) -> Result<S::Ok, S::Error> {
        String::serialize(&base64::encode(Prio2Field::slice_into_byte_vec(v)), s)
    }

    pub fn deserialize_field<'de, D: Deserializer<'de>>(d: D) -> Result<Vec<Prio2Field>, D::Error> {
        let bytes = base64::decode(String::deserialize(d)?.as_bytes()).map_err(Error::custom)?;
        Prio2Field::byte_slice_into_vec(&bytes).map_err(Error::custom)
    }
}
