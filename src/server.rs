...
mod tests {
    ...
    fn tweaks(tweak: Tweak) {
        ...
        // all zero data
        let mut data = vec![Prio2Field::zero(); dim];

        if let Tweak::WrongInput = tweak {
            data[0] = Prio2Field::from(2);
        }
        ...
        let mut share1_field = Prio2Field::byte_slice_into_vec(&decrypted_share1).unwrap();
        let unpacked_share1 = unpack_proof_mut(&mut share1_field, dim).unwrap();

        let one = Prio2Field::from(1);
        ...
        let share1_modified = encrypt_share(
            &Prio2Field::slice_into_byte_vec(&share1_field),
