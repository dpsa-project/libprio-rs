...
/// The Prio2 VDAF. It supports a the same measurement type as
/// [`Prio3Aes128CountVec`](crate::vdaf::prio3::Prio3Aes128CountVec) but uses the proof system
/// deployed in ENPA for input validation. It also uses a different finite field.
#[derive(Clone, Debug)]
pub struct Prio2 {
    input_len: usize,
}
...
mod tests {
    ...
    fn enpa_client_interop() {
        ...
        input_shares.push(Share::get_decoded_with_param(&(&prio2, 0), &input_share1).unwrap());
        input_shares.push(Share::get_decoded_with_param(&(&prio2, 1), &input_share2).unwrap());

        let verify_key = rng.gen();
        let mut nonce = [0; 16];
        rng.fill(&mut nonce);
        run_vdaf_prepare(&prio2, &verify_key, &(), &nonce, input_shares).expect("test failed");
    }
